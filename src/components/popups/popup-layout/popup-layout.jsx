...
export default function PopupLayout({children, isOpen, closeHandler}) {
  ...
  const shouldMount = useDelayUnmountState(!!isOpen, 500);
  const animation = isOpen ? {animation: 'openAniamtion 0.5s ease-out'} : {animation: 'closeAniamtion 0.5s ease-out forwards'}
